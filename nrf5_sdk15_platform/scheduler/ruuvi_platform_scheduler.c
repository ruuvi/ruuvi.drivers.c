...
ruuvi_driver_status_t ruuvi_interface_scheduler_init(size_t event_size, size_t queue_length)
{
  ...
  if(event_size != RUUVI_NRF5_SDK15_SCHEDULER_DATA_MAX_SIZE || queue_length != RUUVI_NRF5_SDK15_SCHEDULER_QUEUE_MAX_LENGTH)
  { RUUVI_DRIVER_ERROR_CHECK(RUUVI_DRIVER_ERROR_INVALID_PARAM, ~RUUVI_DRIVER_ERROR_FATAL); }

  APP_SCHED_INIT(RUUVI_NRF5_SDK15_SCHEDULER_DATA_MAX_SIZE, RUUVI_NRF5_SDK15_SCHEDULER_QUEUE_MAX_LENGTH);
...
ruuvi_driver_status_t ruuvi_interface_scheduler_event_put (void const *p_event_data, uint16_t event_size, ruuvi_scheduler_event_handler_t handler)
{
