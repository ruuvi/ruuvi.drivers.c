...
NRF_QUEUE_DEF(advertisement_t, m_adv_queue, RUUVI_NRF5_SDK15_ADV_QUEUE_LENGTH, NRF_QUEUE_MODE_NO_OVERFLOW);
/** Create queue for incoming advertisements. */
NRF_QUEUE_DEF(ri_adv_scan_t, m_scan_queue, RUUVI_NRF5_SDK15_SCAN_QUEUE_LENGTH, NRF_QUEUE_MODE_NO_OVERFLOW);

static uint16_t m_advertisement_interval_ms; //!< Interval of advertisements, not including random delay by BLE spec.
static uint16_t m_manufacturer_id;           //!< Manufacturer ID for advertisements.
static ri_comm_channel_t * m_channel;        //!< Communication channel interface instance.
                              ...
                              advertisement_t * const p_adv)
{
    ...
    else
    {
        ...
        err_code |= ruuvi_nrf5_sdk15_to_ruuvi_error(
                    ble_advdata_encode (&advdata, p_adv->data.adv_data.p_data,  &(p_adv->data.adv_data.len)));
    }
    return err_code;
}
...
#           endif
        ...
        // Encode data
        err_code |= ble_advdata_encode (&scanrsp, p_adv->data.scan_rsp_data.p_data, &(p_adv->data.scan_rsp_data.len));
    }
    return ruuvi_nrf5_sdk15_to_ruuvi_error (err_code);
}
    ...
    else
    {
        ...
        switch(modulation)
        {
            ...
            case RI_RADIO_BLE_2MBPS:
                p_adv->params.primary_phy = BLE_GAP_PHY_AUTO;
                if(sec_phy_required)
                {
        ...
        switch(m_type)
        {
            case NONCONNECTABLE_NONSCANNABLE:
                ...
                {
                    p_adv->params.properties.type = BLE_GAP_ADV_TYPE_EXTENDED_NONCONNECTABLE_NONSCANNABLE_UNDIRECTED;
                }
                else
                {
            ...
            case NONCONNECTABLE_SCANNABLE:
                if(extended_required)
                {
                    p_adv->params.properties.type = BLE_GAP_ADV_TYPE_EXTENDED_NONCONNECTABLE_SCANNABLE_UNDIRECTED;
                }
                else
                {
            ...
            case CONNECTABLE_NONSCANNABLE:
                if(extended_required)
                {
                    p_adv->params.properties.type = BLE_GAP_ADV_TYPE_EXTENDED_CONNECTABLE_NONSCANNABLE_UNDIRECTED;
                }
                else
                {
