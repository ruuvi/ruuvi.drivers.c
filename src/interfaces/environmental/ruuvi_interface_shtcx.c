...
#define SHTCX_USEC_TO_MSEC      1000        //!< Usecond to msecond converter value

#define SHTCX_SENS_NUM          2           //!< Number of sensor values
#define SHTCX_HUM_TO_F_COEF     (1000.0f)   //!< Humudity to float converter value
#define SHTCX_TEMP_TO_F_COEF    (1000.0f)   //!< Temperature to float converter value

#define SHTCX_INIT_I2C_RETRIES  5           //!< Num od retries to init i2c


/** @brief Function for checking that sensor is in sleep mode before configuration */
static rd_status_t shtcx_verify_sensor_sleep (void)
{
    rd_status_t err_code = RD_SUCCESS;
    uint8_t mode = 0;
    ri_shtcx_mode_get (&mode);
...
static rd_status_t shtcx_success_on_valid (uint8_t param)
{
...
static rd_status_t SHTCX_TO_RUUVI_ERROR (const int16_t rslt)
{
    ...
    else
    {
        ...
        if (STATUS_ERR_BAD_DATA == rslt)  { err_code = RD_ERROR_INVALID_DATA; }

        if (STATUS_CRC_FAIL == rslt)      { err_code = RD_ERROR_INVALID_DATA; }

        if (STATUS_WAKEUP_FAILED == rslt) { err_code = RD_ERROR_INTERNAL; }

        if (STATUS_SLEEP_FAILED == rslt)  { err_code = RD_ERROR_INTERNAL; }
    }

    return err_code;
}

static rd_status_t ri_shtcx_init_bus (rd_bus_t bus)
{
    rd_status_t err_code = RD_SUCCESS;
    size_t retries = 0;
    ...
    switch (bus)
    {
        case RD_BUS_I2C:
            do
            {
                ...
                retries++;
            } while ( (RD_ERROR_INVALID_DATA == err_code) && (retries < SHTCX_INIT_I2C_RETRIES));
                           ...
                           environmental_sensor, rd_bus_t bus, uint8_t handle)
{
    ...
    else
    {
        ...
        else
        {
            ...
            err_code = ri_shtcx_init_bus (bus);
...
rd_status_t ri_shtcx_resolution_set (uint8_t * resolution)
{
    ...
    else
    {
        ...
        *resolution = RD_SENSOR_CFG_DEFAULT;
        shtcx_success_on_valid (original);
    }
...
static rd_status_t ri_shtcx_mode_set_single (uint8_t * mode)
{
    rd_status_t err_code = RD_ERROR_INVALID_PARAM;
...
rd_status_t ri_shtcx_mode_set (uint8_t * mode)
{
    ...
    else
    {
        ...
        else
        {
            ...
            {
                err_code = ri_shtcx_mode_set_single (mode);
            }
                               ...
                               p_data)
{
    ...
    else
    {
        ...
        if ( (RD_SUCCESS == err_code) && (RD_UINT64_INVALID != m_tsample))
        {
            ...
            rd_float env_values[SHTCX_SENS_NUM];
            env_values[0] =  m_humidity / SHTCX_HUM_TO_F_COEF;
            env_values[1] =  m_temperature / SHTCX_TEMP_TO_F_COEF;
...
{
    if (useconds < SHTCX_USEC_TO_MSEC)
    {
        ri_delay_us (useconds);
    }
    else
    {
        ri_delay_ms ( (useconds / SHTCX_USEC_TO_MSEC) + 1);
    }
}
