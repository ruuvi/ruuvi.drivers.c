...
static bool initialize_sensor_once (ruuvi_driver_sensor_t * DUT,
                                    const ruuvi_driver_sensor_init_fp init,
...
static bool uninitialize_sensor (ruuvi_driver_sensor_t * DUT,
                                 const ruuvi_driver_sensor_init_fp init,
                                 const ruuvi_driver_bus_t bus, const uint8_t handle)
{
    ruuvi_driver_status_t err_code = RUUVI_DRIVER_SUCCESS;
...
static bool initialize_sensor_twice (ruuvi_driver_sensor_t * DUT,
                                     const ruuvi_driver_sensor_init_fp init,
                                     const ruuvi_driver_bus_t bus, const uint8_t handle)
{
    ruuvi_driver_status_t err_code = RUUVI_DRIVER_SUCCESS;
...
{
    bool failed = false;
    failed |= (RUUVI_DRIVER_ERROR_NOT_INITIALIZED != DUT->init (NULL, 0,
               0))                 ? true : false;
    failed |= (RUUVI_DRIVER_ERROR_NOT_INITIALIZED != DUT->uninit (NULL, 0,
               0))               ? true : false;
    failed |= (RUUVI_DRIVER_ERROR_NOT_INITIALIZED != DUT->configuration_get (NULL,
               NULL))    ? true : false;
    failed |= (RUUVI_DRIVER_ERROR_NOT_INITIALIZED != DUT->configuration_set (NULL,
               NULL))    ? true : false;
    failed |= (RUUVI_DRIVER_ERROR_NOT_INITIALIZED != DUT->data_get (
                   NULL))                   ? true : false;
    failed |= (RUUVI_DRIVER_ERROR_NOT_INITIALIZED != DUT->dsp_get (NULL,
               NULL))              ? true : false;
    failed |= (RUUVI_DRIVER_ERROR_NOT_INITIALIZED != DUT->dsp_set (NULL,
               NULL))              ? true : false;
    failed |= (RUUVI_DRIVER_ERROR_NOT_INITIALIZED != DUT->fifo_enable (
                   false))               ? true : false;
    failed |= (RUUVI_DRIVER_ERROR_NOT_INITIALIZED != DUT->fifo_interrupt_enable (
                   false))     ? true : false;
    failed |= (RUUVI_DRIVER_ERROR_NOT_INITIALIZED != DUT->fifo_read (NULL,
               NULL))            ? true : false;
    failed |= (RUUVI_DRIVER_ERROR_NOT_INITIALIZED != DUT->level_interrupt_set (false,
               NULL)) ? true : false;
    failed |= (RUUVI_DRIVER_ERROR_NOT_INITIALIZED != DUT->mode_get (
                   NULL))                   ? true : false;
    failed |= (RUUVI_DRIVER_ERROR_NOT_INITIALIZED != DUT->mode_set (
                   NULL))                   ? true : false;
    failed |= (RUUVI_DRIVER_ERROR_NOT_INITIALIZED != DUT->resolution_get (
                   NULL))             ? true : false;
    failed |= (RUUVI_DRIVER_ERROR_NOT_INITIALIZED != DUT->resolution_set (
                   NULL))             ? true : false;
    failed |= (RUUVI_DRIVER_ERROR_NOT_INITIALIZED != DUT->samplerate_get (
                   NULL))             ? true : false;
    failed |= (RUUVI_DRIVER_ERROR_NOT_INITIALIZED != DUT->samplerate_set (
                   NULL))             ? true : false;
    failed |= (RUUVI_DRIVER_ERROR_NOT_INITIALIZED != DUT->scale_get (
                   NULL))                  ? true : false;
    failed |= (RUUVI_DRIVER_ERROR_NOT_INITIALIZED != DUT->scale_set (
                   NULL))                  ? true : false;
...
static bool test_sensor_init_on_null (ruuvi_driver_sensor_t * DUT,
                                      const ruuvi_driver_sensor_init_fp init,
                                      const ruuvi_driver_bus_t bus, const uint8_t handle)
{
...
static bool sensor_returns_invalid_before_sampling (const ruuvi_driver_sensor_t * const
        DUT)
{
    ruuvi_driver_status_t err_code = RUUVI_DRIVER_SUCCESS;
