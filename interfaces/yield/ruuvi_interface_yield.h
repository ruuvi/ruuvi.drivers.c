 ...
 * If there are no other threads or we're running in a single-threaded invironment, go to sleep
...
/** 
  ...
  **/
ruuvi_status_t platform_yield_set(yield_fptr_t yield_ptr);
...
/** 
  ...
  * Return RUUVI_SUCCESS on success, error code otherwise.
  **/
ruuvi_status_t platform_delay_ms(uint32_t time);
...
/** 
  ...
  * Return RUUVI_SUCCESS on success, error code otherwise.
  **/
ruuvi_status_t platform_delay_us(uint32_t time);
